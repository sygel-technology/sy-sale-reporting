...
class SaleOrder(models.Model):
    ...
    hide_tax_information = fields.Boolean(
        ...
        compute="_compute_hide_tax_information"
    )

    def _compute_has_show_price_report_lines(self):
        for sel in self:
            sel.has_show_price_report_lines = any(
                line.show_price_info_report for line in sel.order_line.filtered(
    ...
    def _compute_hide_tax_information(self):
        for sel in self:
            sel.hide_tax_information = self.env[
                'ir.config_parameter'
            ].sudo().get_param(
                'so_report_hide_price.hide_taxes'
            )
